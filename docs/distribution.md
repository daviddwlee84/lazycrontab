...
Version **v0.1.3** provides macOS and Linux binaries for amd64 and arm64. Go is
...
For standalone installs, download the matching `lazycrontab_VERSION_OS_ARCH.tar.gz`
from [GitHub Releases](https://github.com/daviddwlee84/lazycrontab/releases), verify its
...
candidates and refreshes completions. The existing source updater also supports versioned Go installations; it
uses the published stable release tag and stages a build with existing Go.
Release/local binaries remain preserved by that source path. Never overwrite a Homebrew keg manually or
...
go install github.com/daviddwlee84/lazycrontab@latest
```
...
Source tags recover their module version; release archives inject v0.1.3; local
...
python3 scripts/release.py check --project lazycrontab --binary lazycrontab --smoke
